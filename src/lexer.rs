...
pub fn tokenize(code: &str) -> Vec<String> {
    ...
    let token_spec = r#"("[^"]*"|'[^']*'|==|!=|<=|>=|<|>|&&|\|\||\d+|വായിക്കുക|എഴുതുക|കൂട്ടിച്ചേർക്കുക|പറയുക|സ്വീകരിക്കുക|മുതൽ|വരെ|ഈ|സത്യമാവണവരെ|ഇത്|സത്യമോ|അല്ലെങ്കില്|നിശ്ചയിക്കുക|മറുപടി|മുറിക്കുക|[a-zA-Z_]\w*|[\[\].]|[%+\-*/=(),{}])"#;
